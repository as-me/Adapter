...
class WeavePanel {
    ...
    constructor(parent, toolPath, ToolClass) {
        ...
        ["panelY", "panelX", "panelHeight", "panelWidth", "maximized", "zOrder", "panelBorderColor"].forEach(
            (item) => {
                this.toolPath.push(item).addCallback(boundFunc, true, false);
            }, this);
    ...
    _panelChanged() {
        ...
        this.element.css("position", "absolute")
            .css({
                top, left, height, width, "max-height": height, "max-width": width
            })
            .css({
                "-webkit-transform": "translateZ(0)"
            }); /* In Webkit, forces the div to be a compositing layer. */
